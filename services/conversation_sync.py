...
from loaders.db_loader import get_db_info
...
class ConversationSyncManager:
    ...
    def _sync_single_conversation(self, conversation_id: str) -> Dict:
        ...
        try:
            ...
            print(f"🔍 **Auth Debug:** Using session cookies (like your working code)")
            print(f"🔍 **Main Cookie:** .AspNetCore.Identity.Application present: {bool(cookies.get('.AspNetCore.Identity.Application'))}")
            ...
            print(f"Key cookie present: {bool(cookies.get('.AspNetCore.Identity.Application'))}")
            ...
            return {
                ...
                "timestamp": datetime.datetime.now(),
                "status": f"✅ Synced - {added} new messages" if added > 0 else "✅ Up to date",
                "debug_info": {
                    **debug_info,
                    "response_status": response.status_code,
    ...
    def _add_messages_to_database(self, messages: list, conversation_id: str) -> int:
        ...
        for msg in messages:
            try:
                ...
                brt_dt = None
                if ts:
                    brt_dt = datetime.datetime.fromtimestamp(ts, tz=datetime.timezone.utc)
                    brt_dt = brt_dt.astimezone(pytz.timezone("America/Sao_Paulo")).replace(tzinfo=None)
                ...
                cur.execute(
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?)
                    """,
                    [msg_id, conversation_id, ts, brt_dt, sender, text, from_me, m_type, is_biz],
                )
                if cur.rowcount:
                    added += 1
            except Exception:
        ...
        with sqlite3.connect(LOCAL_DB_PATH) as conn:
            ...
            cur.execute(
                ...
                [conversation_id, conversation_id, conversation_id, 
                 int(datetime.datetime.now().timestamp()), conversation_id],
            )
